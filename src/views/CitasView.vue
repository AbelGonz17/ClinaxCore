...
import NuevaCitaModal from "@/components/Citas/NuevaCitaModal.vue";
import {
  ChevronLeft,
  ChevronRight,
  Calendar,
...
const { citasDelDia, resumenDelDia, fechaFormateada, esHoy, filtros } =
  storeToRefs(agendaStore);
const mostrarModalNuevaCita = ref(false);

function iniciarConsulta(citaId: string) {
  const cita = agendaStore.citas.find(c => c.id === citaId);
  if (cita) {
    consultaStore.formData.nombre = cita.pacienteNombre;
    if (cita.edad) consultaStore.formData.edad = cita.edad;
    if (cita.telefonoCuidador) consultaStore.formData.telefonoCuidadora = cita.telefonoCuidador;
    consultaStore.formData.motivoConsulta = cita.motivo;
    consultaStore.formData.fechaEvaluacion = new Date().toISOString().split('T')[0];
  }
  ...
  <div class="space-y-6">
    ...
    >
      ...
      <div class="flex items-center gap-3">
        ...
        <div
          class="flex items-center bg-white rounded-lg border border-neutral-200 shadow-sm"
        >
          <button
          ...
          <div
            class="px-4 py-2 border-x border-neutral-200 flex items-center gap-2"
          >
        ...
        <!-- View selector -->
        <div
          class="hidden sm:flex items-center bg-white rounded-lg border border-neutral-200 shadow-sm p-1"
        >
          <button
        ...
        <!-- Nueva Cita button -->
        <Button @click="mostrarModalNuevaCita = true" size="sm">
          <Plus class="w-4 h-4" />
          Nueva cita
        </Button>
      </div>
    ...
    >
      ...
      <!-- Empty State -->
      <div v-if="citasDelDia.length === 0" class="py-16 px-4 text-center">
        <div
        ...
        <p class="text-neutral-500">
          No hay citas para este día. Selecciona otra fecha o agenda una nueva
          cita.
        </p>
      </div>
